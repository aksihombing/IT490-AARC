...
function doBookSearch(array $req)
{
  ...
  foreach ($curl_data['docs'] as $book) {
      ...
      // even though openlibrary's documentation says theres an "isbn" tag, most docs dont actually have it set.
      // "ia" [] has data starting with "isbn_111111111" so we could use the first entry from that?
      //$isbn = $book['isbn'][0] ?? null; //string
      $isbn_search = array_search(str_starts_with('isbn_'), $book['ia']);
      ...
      // SUBJECT/GENRE INFO
      // search.json actually doesnt have these
      // use OLID to use subject.json and grab subjects from here
